...
from subscriptions.serializers import LessonAttendanceSerializer, PaymentSerializer, SubscriptionSerializer
...
def build_dashboard_calendar(selected_date):
...
def build_dashboard_payload(selected_date=None):
    ...
    payments_on_date = (
        Payment.objects.filter(status="completed", paid_at__date=selected_date)
        .select_related("subscription", "subscription__tariff", "subscription__student")
    )
    ...
    for payment in payments_on_date:
        is_group_subscription = StudentGroups.objects.filter(
            student=payment.subscription.student,
            group__course=payment.subscription.tariff.course,
        ).exists()
    ...
    return {
        ...
        "calendar": build_dashboard_calendar(selected_date),
        "stats": {
